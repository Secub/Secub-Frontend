...
function addLogToCache(correlationId, loggedMessage) {
	...
	if (data) markAsRecentlyUsed(correlationId, data);
		...
		if (correlationCache.size > CACHE_CAPACITY) {
			...
			if (firstKey) correlationCache.delete(firstKey);
		}
	}
	data.logs.push({
		...loggedMessage,
...
var BrowserCacheManager = class extends CacheManager {
	...
	*/
	trackVersionChanges(correlationId) {
		...
		if (previousVersion !== "5.8.0") this.setItem(VERSION_CACHE_KEY, version, correlationId);
	}
	/**
	...
	async acquireTokenRedirect(request, rootMeasurement, options) {
		...
		try {
			...
			if (e instanceof NativeAuthError) {
				initializeServerTelemetryManager(this.apiId, this.config.auth.clientId, this.correlationId, this.browserStorage, this.logger).setNativeBrokerErrorCode(e.errorCode);
	...
	*/
	async handleRedirectPromise() {
		...
		try {
			...
			initializeServerTelemetryManager(this.apiId, this.config.auth.clientId, this.correlationId, this.browserStorage, this.logger).clearNativeBrokerErrorCode();
...
function buildConfiguration({ auth: userInputAuth, cache: userInputCache, system: userInputSystem, experimental: userInputExperimental, telemetry: userInputTelemetry }, isBrowserEnvironment) {
	...
	const DEFAULT_EXPERIMENTAL_OPTIONS = { iframeTimeoutTelemetry: false };
...
var PopupClient = class extends StandardInteractionClient {
	...
	*/
	async executeEarFlow(request, popupParams, pkceCodes) {
		...
		(await getEARForm((popupParams.popup || this.openPopup("about:blank", popupParams)).document, this.config, discoveredAuthority, popupRequest, this.logger, this.performanceClient)).submit();
	...
	async executeCodeFlowWithPost(request, popupParams, authClient, pkceVerifier) {
		...
		(await getCodeForm((popupParams.popup || this.openPopup("about:blank", popupParams)).document, this.config, discoveredAuthority, request, this.logger, this.performanceClient)).submit();
	...
	*/
	async handleRedirectPromise(request, pkceVerifier, parentMeasurement, options) {
		...
		try {
			...
			if (normalizeUrlForComparison(loginRequestUrl) === normalizeUrlForComparison(window.location.href) && navigateToLoginRequestUrl) {
...
var SilentIframeClient = class extends StandardInteractionClient {
	...
	*/
	async silentAuthorizeHelper(authClient, request) {
		...
		let iframe;
		if (request.httpMethod === HttpMethod.POST) iframe = await invokeAsync(initiateCodeFlowWithPost, SilentHandlerInitiateAuthRequest, this.logger, this.performanceClient, correlationId)(this.config, authClient.authority, silentRequest, this.logger, this.performanceClient);
		else {
...
function getPerfMeasurementModule() {
	...
	try {
		...
		if (Number(perfEnabled) === 1) return import("./BrowserPerformanceMeasurement-CQoGzS2q.js").then((n) => n.n);
	} catch (e) {}
}
/**
...
//#endregion
export { BrowserCacheLocation as A, InteractionRequiredAuthError as B, MemoryStorage as C, BrowserConfigurationAuthError as D, BrowserConfigurationAuthErrorCodes_exports as E, BrowserAuthError as F, LogLevel as G, StubPerformanceClient as H, AuthErrorCodes_exports as I, ClientConfigurationErrorCodes_exports as J, Logger as K, AuthenticationHeaderParser as L, InteractionStatus as M, InteractionType as N, BrowserAuthErrorCodes_exports as O, WrapperSKU as P, enforceResourceParameter as R, BrowserRootPerformanceEvents_exports as S, createGuid as T, ProtocolMode as U, InteractionRequiredAuthErrorCodes_exports as V, AzureCloudInstance as W, ClientConfigurationError as X, ClientAuthError as Y, AuthError as Z, EventHandler as _, ResponseMode as a, SessionStorage as b, EventMessageUtils as c, PublicClientApplication as d, createNestablePublicClientApplication as f, NavigationClient as g, DEFAULT_IFRAME_TIMEOUT_MS as h, PromptValue as i, CacheLookupPolicy as j, ApiId as k, loadExternalTokens as l, isPlatformBrokerAvailable as m, JsonWebTokenTypes as n, BrowserPerformanceClient as o, createStandardPublicClientApplication as p, ClientAuthErrorCodes_exports as q, OIDC_DEFAULT_SCOPES as r, SignedHttpRequest as s, AuthenticationScheme as t, stubbedPublicClientApplication as u, version as v, BrowserUtils_exports as w, LocalStorage as x, EventType as y, ServerError as z };

//# sourceMappingURL=dist-iTqYjVbn.js.map
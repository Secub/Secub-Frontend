...
export default function ProgramSelectionSection({
  ...
  return (
    ...
    >
      ...
      <div className="mt-7 grid gap-3" aria-label="Programas académicos disponibles">
        {secubAcademicPrograms.length === 0 ? (
          ...
          secubAcademicPrograms.map((program) => (
            <button
              key={program.id}
              type="button"
              className="group rounded-[var(--radius-xl)] border border-[var(--color-gray-6)] bg-[var(--color-surface-soft)] p-4 text-left transition-all hover:-translate-y-0.5 hover:border-[var(--color-secondary-2)] hover:bg-white hover:shadow-[0_18px_45px_rgba(24,34,51,0.10)] focus:outline-none focus-visible:ring-4 focus-visible:ring-[color:rgba(14,101,217,0.24)]"
              onClick={() => onSelectProgram(program.id)}
            >
              <span className="flex items-center justify-between gap-4">
                <span className="flex min-w-0 items-center gap-3">
                  <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[var(--radius-lg)] bg-[var(--color-secondary-1)] text-xl text-white">
                ...
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[var(--radius-pill)] border border-[var(--color-gray-6)] text-[var(--color-secondary-4)] transition-all group-hover:border-[var(--color-secondary-2)] group-hover:bg-[var(--color-secondary-1)] group-hover:text-white">
